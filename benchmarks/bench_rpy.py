...
def two_body_rpy_batch(
    rel_vecs: torch.Tensor,
    radii: torch.Tensor,
) -> torch.Tensor:
    """
    Copied from _two_body_mu_batch(), this one only takes relative vectors,
    not absolute centres.
    
    rel_vecs = centres[:, 0, :] - centres[:, 1, :]
    """
    device = rel_vecs.device
    dtype = rel_vecs.dtype
    batch = rel_vecs.shape[0]

    # assert 32 bit dtype
    assert dtype == torch.float32, "Expected float32 dtype"

    distances = torch.linalg.norm(rel_vecs, dim=-1)
    inv_r = 1.0 / distances
    inv_r2 = inv_r ** 2
    inv_r3 = inv_r ** 3
    r_hat = rel_vecs / distances.unsqueeze(-1)
...
def accuracy_test() -> None:
    ...
    for idx, (centres_list, radii_list) in enumerate(test_cases):
        ...
        n = centres.shape[0]
        if n == 2:
            rel_vecs = (centres[0] - centres[1]).unsqueeze(0)
            radii_batch = radii.unsqueeze(0)
            ours = two_body_rpy_batch(rel_vecs, radii_batch)[0]
        else:
            ours = mu(centres, radii)
