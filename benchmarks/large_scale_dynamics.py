...
VIDEO_PADDING_FRACTION = np.array([0.05, 0.05, 0.2])
VIDEO_PADDING_MIN = np.array([0.5, 0.5, 1.5])
VIDEO_PADDING_MAX = np.array([6.0, 6.0, 12.0])
CAMERA_DIST_BASE = 9.0
CAMERA_DIST_MIN = 6.5
CAMERA_DIST_MAX = 11.0
PARAVIEW_SERIES_SUFFIX = "_paraview"
...
def build_mobility_operator(name: str) -> Mob_Nbody_Torch:
...
def save_simulation_video(
    ...
    *,
    ...
    padding = span_vec * VIDEO_PADDING_FRACTION
    padding = np.clip(padding, VIDEO_PADDING_MIN, VIDEO_PADDING_MAX)
...
def save_snapshots(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError(
            "Snapshot export requires matplotlib. Install it via `pip install matplotlib` and retry."
        ) from exc

    output_dir.mkdir(parents=True, exist_ok=True)
    # Compute bounding box across ALL timesteps to use the same box for all snapshots
    flat_positions = positions.reshape(-1, 3)
    mins = flat_positions.min(axis=0)
    maxs = flat_positions.max(axis=0)
    
    # Add small padding per dimension (tighter than video)
    padding_xy = 2.0  # Smaller padding for x and y
    padding_z_top = 2.0   # Small padding at top
    padding_z_bottom = 5.0  # More padding at bottom to show settling progress
    
    # Create box with asymmetric z-padding to show settling
    lims_vis = np.array([
...
def _write_vtp_frame(path: Path, points: np.ndarray, velocities: np.ndarray | None) -> None:
    """Write a single PolyData VTP file containing points (and optional velocities)."""
    ...
    with path.open("w", encoding="utf-8") as handle:
        ...
        if velocities is not None:
            handle.write('      <PointData Vectors="velocity">\n')
            handle.write(
                '        <DataArray type="Float32" Name="velocity" NumberOfComponents="3" format="ascii">\n'
            )
            for row in velocities:
                handle.write(
                    f"          {row[0]:.6e} {row[1]:.6e} {row[2]:.6e}\n"
                )
        ...
        for row in points:
            handle.write(f"          {row[0]:.6e} {row[1]:.6e} {row[2]:.6e}\n")
        handle.write("        </DataArray>\n")
        handle.write("      </Points>\n")
        handle.write("      <Verts>\n")
...
def export_paraview_series(
    ...
    *,
    ...
    """Export every frame as a VTP file plus a PVD manifest for ParaView playback."""

    positions = np.asarray(positions)
    velocities = np.asarray(velocities)
    times = np.asarray(times) if times is not None else None
    frame_count = positions.shape[0]
    series_dir = base_output.parent / f"{base_output.stem}{PARAVIEW_SERIES_SUFFIX}"
    series_dir.mkdir(parents=True, exist_ok=True)
    print(f"Writing ParaView frames to {series_dir} ...")
    vtp_names = []
    for idx in range(frame_count):
        frame_name = f"frame_{idx:05d}.vtp"
        frame_path = series_dir / frame_name
        frame_vel = velocities[idx] if idx < velocities.shape[0] else None
        _write_vtp_frame(frame_path, positions[idx], frame_vel)
        vtp_names.append(frame_name)

    pvd_path = series_dir / "series.pvd"
    with pvd_path.open("w", encoding="utf-8") as handle:
        handle.write("<?xml version=\"1.0\"?>\n")
        handle.write(
            "<VTKFile type=\"Collection\" version=\"0.1\" byte_order=\"LittleEndian\">\n"
        )
        handle.write("  <Collection>\n")
        for idx, name in enumerate(vtp_names):
            timestep = float(times[idx]) if times is not None and idx < len(times) else float(idx)
            handle.write(
                f'    <DataSet timestep="{timestep:.6f}" group="" part="0" file="{name}"/>\n'
            )
        handle.write("  </Collection>\n")
        handle.write("</VTKFile>\n")
    print(f"ParaView collection written to {pvd_path}")
    return pvd_path
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--yes-plot", action="store_true", help="Skip the PyVista preview window")
    parser.add_argument(
        "--off-screen",
        action="store_true",
...
def main() -> None:
    ...
    if args.replay_only:
        ...
        if not args.no_video:
            snapshot_dir = SIM_OUTPUT_PATH.parent
            try:
        ...
        # export_paraview_series(
        #     positions,
        #     velocities,
        #     times,
        #     base_output=args.replay_from,
        # )
